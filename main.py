...
selected = st.sidebar.selectbox("Select the section", [
    'Introduction', 'Admin', 'Exploration', 'Verification', 'Graph', 'Prediction', 'Recommendation'])
...
if selected == 'Introduction':
    with siteHeader:
        ...
        st.markdown('Christopher Toromo')
        st.markdown('Anne Nyambura')
        st.markdown('Cynthia Mwadime')
        st.markdown('Elizabeth Kihungi')
        st.markdown('John Njagi')
        st.markdown('Matilda Kadzo')
        st.markdown('Stephen Odhiambo')
        st.markdown('Ruth Mwaura')
    ...
    credit = st.number_input("Credit Score:", key=251)
    income = st.number_input("Annual Income:", key=256)
    jobyrs = st.slider('Years in curent Job', min_value=0,
                       max_value=150, value=2, step=1)
    debt = st.number_input("Monthly Dept:", key=260)
    purpose = st.selectbox(
        'Purpose', ('Debt Consolidation', 'Home Improvements'))
    st.write('You selected:', purpose)

    homeOwn = st.selectbox('Home Ownerhip', ('Home Mortgage', 'Rent'))
    st.write('You selected:', homeOwn)
    ...
    with modelTraining:
        ...
        st.markdown(
            'The data set does not require much cleaning. Detailed cleaning may be done during data preparation')
